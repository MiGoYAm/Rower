...
use crate::protocol::{packet::{Packet, NextPacket, RawPacket, handshake::Handshake, Lazy, status::{StatusRequest, StatusResponse, Ping}, login::{Disconnect, LoginStart, LoginSuccess, SetCompression}}, ProtocolVersion, Direction};

use super::registry::PacketProducer;

struct Mape {
    t: TypeId,
    packet_producer: PacketProducer,
    id: Id
}
...
pub static HANDSHAKE_REG: sync::Lazy<StateRegistry> = sync::Lazy::new(|| {
    ...
    registry.insert::<Handshake>(
        |b, v| NextPacket::Handshake(Lazy::new(b, v)), 
...
pub static STATUS_REG: sync::Lazy<StateRegistry> = sync::Lazy::new(|| {
    ...
    registry.insert::<StatusRequest>(
        |_, _| NextPacket::StatusRequest, 
        Id::Serverbound(Mapping::Single(0x00))
    );
    registry.insert::<StatusResponse>(
        |b, v| NextPacket::StatusResponse(Lazy::new(b, v)), 
        Id::Clientbound(Mapping::Single(0x00))
    );
    registry.insert::<Ping>(
        |b, v| NextPacket::Ping(Lazy::new(b, v)), 
...
pub static LOGIN_REG: sync::Lazy<StateRegistry> = sync::Lazy::new(|| {
    ...
    registry.insert::<Disconnect>(
        |b, v| NextPacket::Disconnect(Lazy::new(b, v)), 
        Id::Clientbound(Mapping::Single(0x00))
    );
    registry.insert::<LoginStart>(
        |b, v| NextPacket::LoginStart(Lazy::new(b, v)), 
        Id::Serverbound(Mapping::Single(0x00))
    );
    registry.insert::<LoginSuccess>(
        |b, v| NextPacket::LoginSuccess(Lazy::new(b, v)), 
        Id::Clientbound(Mapping::Single(0x02))
    );
    registry.insert::<SetCompression>(
        |b, v| NextPacket::SetCompression(Lazy::new(b, v)), 
